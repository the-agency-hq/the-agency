 ...
 * GitLab ({@code POST /oauth/token}) and Bitbucket ({@code POST /site/oauth2/access_token}) answer with the same
 * member names, so one record reads all three; the members a host does not send are {@code null}.
 *
 * <p>GitHub answers HTTP 200 for both outcomes: a successful exchange carries {@code access_token}, and a failed one
 * carries {@code error} and no token. GitLab and Bitbucket answer a failure with a 4xx and the same {@code error}
 * members. So a
 ...
 *                              is none. GitHub sends it; GitLab's and Bitbucket's refresh tokens carry no expiry.
 */
