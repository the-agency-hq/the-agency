 ...
 * {@code null} for all of them — the default, and the state every Organization starts in. The Organization holds
 * no credential of any kind: its Brief source, and whatever authorization that source needs, is the
 * {@link BriefSource} row keyed to it.
 *
 * <p>One type rather than a domain record and a wire twin. Inside a Brief only {@code id}, {@code name} and
 * {@code agents} travel: {@code BriefBuilder} nulls the instants, which serialization then omits, because a
 * timestamp that moves on every write would poison the Brief's content
 ...
 */
@JSON
public record Organization(UUID id, String name, Agents agents, Instant insertInstant, Instant updateInstant) {
  public Organization {
